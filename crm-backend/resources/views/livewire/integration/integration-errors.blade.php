                ...
                <div class="bg-red-50 border border-red-200 rounded-xl p-4 shadow-sm">
                    <div class="flex items-start justify-between">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center space-x-2 mb-1">
                                    <h4 class="text-sm font-semibold text-red-800">
                                        {{ $account['provider'] }} Synchronization Error
                                    </h4>
                                ...
                                @if($account['last_error_at'])
                                    <p class="text-xs text-red-600">
                                        Last error: {{ $account['last_error_at'] }}
                                    </p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-2 ml-4">
                            @if($account['can_retry'])
                                <button 
                                    wire:click="retryAccount({{ $account['id'] }})"
                                    ...
                                    wire:target="retryAccount({{ $account['id'] }})"
                                >
                                    <svg wire:loading.remove wire:target="retryAccount({{ $account['id'] }})" class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                    </svg>
                                    <svg wire:loading wire:target="retryAccount({{ $account['id'] }})" class="animate-spin w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span wire:loading.remove wire:target="retryAccount({{ $account['id'] }})">Retry</span>
                                    <span wire:loading wire:target="retryAccount({{ $account['id'] }})">Starting...</span>
                                </button>
                            @endif
                            
                            <button 
                                wire:click="dismissError({{ $account['id'] }})"
